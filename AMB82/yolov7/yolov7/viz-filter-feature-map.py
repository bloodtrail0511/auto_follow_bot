...
if __name__ == "__main__":
    ...
    model = torch.load('runs/train/car_v9/weights/best.pt')
    ...
    img = cv.imread('car_v9/IMG_7503.JPG')
    ...
    #                             transforms.ToTensor()])
    # transform = transforms.Compose([transforms.ToPILImage(),
    #                             transforms.Resize((640, 640)), # Can change
    #                             transforms.ToTensor()])
    # transform = transforms.Compose([transforms.ToPILImage(),
    #                             transforms.Resize((320, 576)), # Can change
    #                             transforms.ToTensor()])
    transform = transforms.Compose([transforms.ToPILImage(),
                                transforms.Resize((480, 640)), # Can change
                                transforms.ToTensor()])
    ...
    im = torch.as_tensor(im, dtype=torch.half)
    # =================================================
    ...
    for i in range(len(results)):
        ...
        fname = 'conv_layer_{}_Feature_Map.png'.format(i)
        plt.savefig(os.getcwd() + '/visualize-filter/feature-map/' + args['name'] + '/' + fname, dpi=600)
    # ==================================================
    # ==============================================================================
